...
// implementation of MyGrid3D

MyGrid3D* MyGrid3D::create(const CCSize& gridSize, CCTexture2D *pTexture, bool bFlipped)
{
...
MyGrid3D* MyGrid3D::create(const CCSize& gridSize)
{
    MyGrid3D *pRet= new MyGrid3D();
...
MyGrid3D* MyGrid3D::create(const CCSize& gridSize,CCRect rect)
{
	MyGrid3D *pRet = new MyGrid3D();
...
void MyGrid3D::calculateVertexPoints(void)
{
    ...
    float originX = 0;
    float originY = 0;
    if(_gridRect.equals(CCRectZero) == false){
        CCSize texSize = _gridRect.size;
        m_obStep.x = texSize.width / m_sGridSize.width;
        m_obStep.y = texSize.height / m_sGridSize.height;
        originX = _gridRect.origin.x;
        originY = _gridRect.origin.y;
    }
    ...
    for (x = 0; x < m_sGridSize.width; ++x)
    {
        for (y = 0; y < m_sGridSize.height; ++y)
        {
            ...
            GLfloat x1 = x * m_obStep.x + originX;
            GLfloat x2 = x1 + m_obStep.x;
            GLfloat y1 = y * m_obStep.y + originY;
...
ccVertex3F MyGrid3D::vertex(const CCPoint& pos)
{
...
ccVertex3F MyGrid3D::originalVertex(const CCPoint& pos)
{
    CCAssert( pos.x == (unsigned int)pos.x && pos.y == (unsigned int) pos.y , "Numbers must be integers");
...
void MyGrid3D::setVertex(const CCPoint& pos, const ccVertex3F& vertex)
{
    CCAssert( pos.x == (unsigned int)pos.x && pos.y == (unsigned int) pos.y , "Numbers must be integers");
    int index = (pos.x * (m_sGridSize.height + 1) + pos.y) * 3;
    float *vertArray = (float*)m_pVertices;
...
void MyGrid3D::setGridRect(CCRect& rect)
{
    _gridRect = rect;
}

CCRect MyGrid3D::getGridRect()
{
    return _gridRect;
}
