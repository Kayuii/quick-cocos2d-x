--[[
   对动作的一些封装，觉得蛮好用，搬过来
   该代码是quickcocos2dx3.3final的扩展代码
]]
local cca = {}
_G.cca = cca
...
function cca.seqEx(acts)
   return transition.sequence(acts);
end
...
function cca.spawnEx(acts)
   local array = CCArray:create();
   if acts and next(acts) then
      for k,v in ipairs(acts) do
         array:addObject(v);
      end
   end
   return cca.spawn(array);
end
