...
function UIScrollView:onTouch_(event)
	...
	elseif "moved" == event.name then
		...
		if self._banFlag then
         ...
         --add by shi  垂直模式的时候  水平滑动 不要响应
         if self.direction == UIScrollView.DIRECTION_VERTICAL then
         local dx = event.x - self.prevX_
         local dy = event.y - self.prevY_
         --local tan = dx / dy
         local angle = math.atan2(dy, dx) * 180 / 3.141592654
         angle = math.abs(angle)
         --print("ScrollView angle= ", angle)
         if  angle < 45  or  (angle <= 180 and angle > 135 )  or math.abs(dy - dx) < 25 then
         	return
         end
